...
/**
 * The catalog section name for one delegation tool.
 *
 * Deliberately namespaced under the tool rather than the official
 ...
 * even when a deployment renames this plugin's tool to `subagent`. Deriving it
 * from the tool name — rather than hardcoding one — is what lets a profile mount
 * a SECOND row (another transport provider) without the second row's
 * registration silently throwing and leaving its roles unadvertised.
 * @param toolName - the configured delegation tool name.
 * @returns the section name.
 */
export function catalogSectionName(toolName) {
  return `${toolName ?? 'subagent_role'}:catalog`
}

/** Default catalog section name (the default tool name). */
export const CATALOG_SECTION_NAME = catalogSectionName('subagent_role')
...
export function apply(ctx, config = {}) {
  ...
  const sectionName = catalogSectionName(toolName)
  const loader = createRoleLoader({
    projectRootMarkers: config.projectRootMarkers,
    projectRootTtlMs: config.projectRootTtlMs,
    dshHome: config.dshHome,
    maxBodyBytes: config.maxBodyBytes,
    personaVariables: config.personaVariables,
  })
  /**
  ...
  if (config.enableListTool === true) {
    ...
    safely(`registering the ${listToolName} diagnostic tool`, () => {
